...
def _format_hits(hits):
    ...
    for h in hits:
        ...
        out.append({"title":f"{law}"+(f" (p.{page})" if page!="" else ""),
                    "url":"","source_type":"legal",
                    "snippet":h.page_content[:400].strip(),"law":law})
    return out
...
def search_law(query,top_k=None):
    ...
    use_mmr    = getattr(config,"LEGAL_USE_MMR",True)
    fetch_k    = getattr(config,"LEGAL_FETCH_K",20)
    lambda_mul = getattr(config,"LEGAL_MMR_LAMBDA",0.5)
    # --- MMR: diverse + relevant (falls back to similarity on any error) ---
    if use_mmr:
        try:
            hits=store.max_marginal_relevance_search(
                query,k=k,fetch_k=max(fetch_k,k),lambda_mult=lambda_mul)
